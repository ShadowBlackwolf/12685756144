/*
	...
	Description:
	Markiert alles von der feuerwehr für Feuerwehr mitglieder
*/
private["_markers","_text","_crew","_rank","_inVehicle","_count"];
_markers = [];
_text = "";
_inVehicle = [];
_crew = "";
...
if(visibleMap) then {
	{
		if(!(_x getVariable["restrained",false]) && (alive _x) && !(_x in _inVehicle)) then {
			if(!isNil {(_x getVariable "Feuerwehr")}) then {
				_rank = switch ((_x getVariable "Feuerwehr")) do {
			...
			if(_vehicle != _x && {!(_vehicle isKindOf "ParachuteBase")}) then {
				_count = count(crew _vehicle);
				{
					if(alive _x && _x != driver _vehicle && _x in units(group player)) then {
						_crew = _crew + format ["%1%2%3",_rank, _x getVariable["realname",name _x], if(_forEachIndex == _count-1) then {""} else {", "}];
				...
				_text = format["[%1] %2 %3",getText(configFile>>"CfgVehicles">>typeOf _vehicle>>"DisplayName"),
					if(!isNull (driver _vehicle) && {(driver _vehicle in units(group player))}) then {
						if(_count == 1) then {
							format["%1%2",_rank,(driver _vehicle) getVariable["realname",name _x]]
						}else{
							format["%1%2%3",_rank,(driver _vehicle) getVariable["realname",name _x], ","]
						};
					}else{""}, _crew];
				_crew = "";
			}else{
				_text = format["%1%2",_rank,_x getVariable["realname",name _x]];
			};
